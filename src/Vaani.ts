...
export default class Vaani {
  ...
  public checkroot(tword: string): any {
    ...
    for (let a: number = tword.length; a > 0; a--) {
      ...
      let qcode: any = self.Oword[paku];
      if (qcode && qcode.length > 0) {
        qcode.forEach((b) => {
          if (b.s != null) return false; //to save time for foreign words and misspelled words
          let code: string = b.t.toString().substring(0, 1);
          let subcode: string = b.t.toString().substring(1);
          let vikuthi: string = self.getviku(viku, code, subcode);
          if (vikuthi != "false" && rule.hasOwnProperty(code)) {
            outp[0] = paku + rule[code];
            outp[1] = " " + self.tranlate(vikuthi);
            return outp;
          }
        });
      }
   ...
   * @param sc Sub Code
   */
  public getviku(v: string, c: string, sc: string): string {
    //used for dictionary
    const self = this;
    let blocks: string = "";
    switch (sc) {
    ...
    for (let d = 0; d < 8; d++) {
      if (blocks.indexOf(d.toString()) === -1) continue; //it will ignore the blocks which are not related
      ...
      for (var b = v.length; b > -1; b--) {
        ...
        try {
          let part = self.Eword[c][d][subpaku];
          if (part != null) {
            ...
            if (part.indexOf("①") != -1 || part.indexOf("②") != -1) {
              ...
              let vikuth: string = self.getviku(subviku, code1, subcode1);
  ...
  public tranlate(code: string): string {
    let translation: any = JSON.parse(
      '{"㚱":"நான்காம் வேற்றுமை(கு)","㚲":"இரண்டாம் வேற்றுமை(ஐ)","㚳":"வேற்றுமை உருபு(இன்)","㚵":"மூன்றாம் வேற்றுமை(உடன்)","㚶":"மூன்றாம் வேற்றுமை(ஓடு)","㚷":"மூன்றாம் வேற்றுமை(ஆல்)","㚸":"வேற்றுமை உருபு(இல்)","㚹":"ஏழாம் வேற்றுமை(இடம்)","㚺":"ஆறாம் வேற்றுமை"}'
    );
